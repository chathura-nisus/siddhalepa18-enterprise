...
class ResPartner(models.Model):
    ...
    def _get_followup_attachments(self, options):
        attachment_ids = options.setdefault('attachment_ids', [])
        followup_line = options.get('followup_line')
        if not options.get('join_invoices', followup_line.join_invoices):
            # Ignore all attachments if join_invoices is False
        ...
        if template_id := options.get('template_id', followup_line.mail_template_id):
            ...
            attachment_ids += template_attachments['attachment_ids']
            ...
            attachment_ids += dynamic_attachments.ids
        ...
        attachment_ids += invoice_attachment_ids
