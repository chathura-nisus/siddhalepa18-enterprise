...
class TestHelpdeskMultyCompany(HelpdeskCommon):
    ...
    def test_team_assignement_resource_multicompany(self):
        '''
            ...
            Test Case:
            ==========
            - have a user with 2 resources in 2 different companies
        ...
        '''
        Team = self.env['helpdesk.team'].with_context(tracking_disable=True)
        Users = self.env['res.users'].with_context(tracking_disable=True)
        company_a = self.env.user.company_id
        company_b = self.env['res.company'].with_context(tracking_disable=True).create({
            "name": "Test Company B",
        })
        user_multi = Users.create({
        ...
        team_a = Team.with_user(self.env.user).create({'name': 'Team A', 'company_id': company_a.id})
        team_a.member_ids = [(4, user_multi.id)]
        start_dt = datetime.now(timezone.utc)
        end_dt = start_dt + timedelta(days=7)
        company_calendar = company_a.resource_calendar_id
        work_intervals_per_resource = team_a._get_working_user_interval(start_dt, end_dt, company_calendar, user_multi)
        self.assertEqual(self.env['resource.resource'].browse(work_intervals_per_resource.keys()).company_id, team_a.company_id,
                        "The computed working intervals should only consider resources of the team company.")
